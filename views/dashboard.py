    ...
    if job:
        ...
        if finishInfo == 'true':
            ...
            if job.status < 4 and not de_method:
                ...
                print "ELSE : Status 4"
                cmd = [rscript_path, rdir_path + 'compute_sRGES_enrichment_rankLines.R', str(job_id), case_path, control_path, de_method,
    ...
    if request.method == 'POST':
        ...
        with open(control_path, "w") as f:
            for control in control_ids:
                f.write(control + '\n')
            f.close()
        cmd = [rscript_path, rdir_path + 'DE_analysis.R', job_id, case_path, control_path, de_method, dz_fc_threshold,
...
def manual_check():
    ...
    if request.method == 'POST':
        ...
        cmd = [rscript_path, rdir_path + 'manual_controls_check.R', job_id, case_path, control_path, debug]
        print cmd
